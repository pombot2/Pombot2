...
def get_settings():
    ...
    return {
        ...
        "admin_user": row[16] if len(row) > 16 and row[16] else "admin",
        "admin_pass": row[17] if len(row) > 17 and row[17] else "admin123"
    }
...
def generate_txn_id(pack_name: str) -> str:
    slug = "".join(c for c in pack_name if c.isalnum()).upper()[:6] or "PACK"
    date_str = time.strftime("%y%m%d")
    rnd = f"{random.randint(100, 9999):04d}"
    return f"FT{rnd}{slug}"
...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    th, td { padding: 14px 10px; text-align: left; border-bottom: 1px solid rgba(255, 255, 255, 0.08); }
    th { color: #94a3b8; font-family: 'Orbitron', sans-serif; font-size: 11px; letter-spacing: 0.5px; }
...
# --- 2. Customer Orders View Page (Matching Screenshot) ---
                ...
                <form action="/admin/save-bot-settings" method="post">
                    <div class="grid-3-col">
                        ...
                        <div class="cyber-card">
                            ...
                            <p style="color: #94a3b8; font-size: 12px;">Screenshots forward to this Telegram ID.</p>
                        </div>
                    </div>
...
# --- 4. Category: Message & Buttons Settings (Without Back Button Edit) ---
