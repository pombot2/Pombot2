...
AUTH_COOKIE_NAME = "nagato_session"
AUTH_TOKEN = "nagato_authenticated_secret_token"
...
def init_db():
    ...
    cols = [
        ...
        "msg_report_issue TEXT", "msg_language TEXT", "admin_user TEXT", "admin_pass TEXT"
    ]
    for col in cols:
        try:
    ...
    if c.fetchone()[0] == 0:
        ...
        default_details = {
            str(i): {
                ...
                "desc": "Instant VIP collection access."
            } for i in range(17)
        }
...
def get_settings():
    ...
    return {
        ...
        "admin_user": row[17] if len(row) > 17 and row[17] else "admin",
        "admin_pass": row[18] if len(row) > 18 and row[18] else "admin123"
    }
...
def register_user(chat_id: int, username: str):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users (chat_id, username) VALUES (?, ?)", (chat_id, username or "Anonymous"))
    conn.commit()
    conn.close()
...
def log_order(txn_id: str, chat_id: int, username: str, pack_name: str, amount: float):
    ...
    c.execute("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, (txn_id, chat_id, username or "Anonymous", pack_name, amount))
    conn.commit()
    conn.close()
...
def get_stats():
    ...
    c.execute("SELECT COUNT(*), COALESCE(SUM(amount), 0.0) FROM orders WHERE status = 'COMPLETED'")
    paid_row = c.fetchone()
    paid_orders = paid_row[0] or 0
    revenue = paid_row[1] or 0.0

    c.execute("SELECT txn_id, chat_id, pack_name, amount, status FROM orders ORDER BY id DESC LIMIT 10")
...
<style>
    ...
    :root {
        --bg-dark: #070913;
        --card-bg: rgba(18, 17, 34, 0.75);
        --card-border: rgba(168, 85, 247, 0.25);
        --card-border-glow: rgba(168, 85, 247, 0.45);
        --cyan-glow: #00f0ff;
        --purple-glow: #c084fc;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar {
        position: fixed;
        top: 0; left: -320px; width: 280px; height: 100%;
        background: rgba(12, 12, 27, 0.96);
    ...
    th { color: #94a3b8; font-family: 'Orbitron', sans-serif; font-size: 11px; }
...
def render_sidebar(active_page="dashboard"):
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-nav">
            ...
            <a href="/admin/packs" class="{'active' if active_page=='packs' else ''}">📦 17 VIP Packs</a>
            <a href="https://t.me/NAGATOxOWNER" target="_blank">✈️ Contact Developer</a>
        </div>
...
# --- 1. Clean Dashboard (Stats & Orders Only) ---
...
async def admin_dashboard(request: Request):
    ...
    if recent_orders:
        for ord in recent_orders:
            st_cls = "status-completed" if ord[4] == "COMPLETED" else "status-pending"
            table_rows += f"""
            <tr>
                <td><code>{ord[0]}</code></td>
                <td>{ord[1]}</td>
                <td>{ord[2]}</td>
                <td>₹{ord[3]:.2f}</td>
                <td class="{st_cls}">{ord[4]}</td>
            </tr>
            """
    else:
        ...
        <div class="main-wrapper">
            ...
            <div class="dashboard-container">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card" style="border: 1px solid rgba(0, 240, 255, 0.4);">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <h4>Engine</h4>
                            <span class="status-badge" style="padding:2px 8px; font-size:9px;">ACTIVE</span>
                        </div>
                        <div class="val" style="font-size: 16px; color: var(--cyan-glow); margin-top: 6px;">{bot_manager.status}</div>
                    </div>
                </div>
                ...
                <div class="cyber-card">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 12px;">
                        ...
                        <span style="font-size:12px; color:#94a3b8;">GATEWAY: DIRECT UPI QR</span>
                    </div>
                ...
                <!-- Category Navigation Shortcuts (3 in a line) -->
                <h3 style="font-family:'Orbitron',sans-serif; font-size:15px; color:var(--cyan-glow); margin-bottom: 12px;">⚡ Manage Categories</h3>
                <div class="grid-3-col">
                    <div class="cyber-card" style="text-align: center;">
                        <h3>⚙️ Bot Engine & UPI</h3>
                        <p style="color: #94a3b8; font-size: 13px; margin-bottom: 16px;">Manage Telegram token, UPI ID, and payment alerts.</p>
                        <a href="/admin/bot-settings" style="text-decoration:none;"><button class="btn-gradient">Open Settings →</button></a>
                    </div>
                    <div class="cyber-card" style="text-align: center;">
                        <h3>💬 Messages & Actions</h3>
                        <p style="color: #94a3b8; font-size: 13px; margin-bottom: 16px;">Configure welcome album, captions, and footer action buttons.</p>
                        <a href="/admin/message-settings" style="text-decoration:none;"><button class="btn-gradient">Open Settings →</button></a>
                    </div>
                    <div class="cyber-card" style="text-align: center;">
                        <h3>📦 17 VIP Packs</h3>
                        <p style="color: #94a3b8; font-size: 13px; margin-bottom: 16px;">Edit 17 green buttons, custom prices, and bulk video sets.</p>
                        <a href="/admin/packs" style="text-decoration:none;"><button class="btn-gradient">Manage Packs →</button></a>
                    </div>
                </div>
            </div>
        </div>
...
# --- 2. Category: Bot & Payment Gateway Settings (3 in a Line) ---
            ...
            <div class="dashboard-container">
                <form action="/admin/save-bot-settings" method="post">
                    ...
                    <div class="grid-3-col">
                        ...
                        <!-- Card 3 -->
                        <div class="cyber-card">
                            <h3>3. Admin Forwarding</h3>
                            <label>Admin Telegram Chat ID:</label>
                            <input type="text" name="admin_chat_id" value="{cfg['admin_chat_id']}" placeholder="e.g. 123456789" />
                            <p style="color: #94a3b8; font-size: 12px;">All user receipt screenshots forward to this Telegram ID.</p>
                        </div>
                    </div>
...
# --- 3. Category: Message & Buttons Settings (3 in a Line) ---
            ...
            <div class="dashboard-container">
                <form action="/admin/save-message-settings" method="post">
                    ...
                    <div class="grid-3-col">
                        ...
                        <div class="cyber-card">
                            <h3>3. Navigation Labels</h3>
                            <label>🔵 Payment Back Button Label:</label>
                            <input type="text" name="back_button_text" value="{cfg['back_button_text']}" required />
                            <p style="color: #94a3b8; font-size: 12px; margin-top: 14px;">Appears at the bottom of the payment details card.</p>
                        </div>
                    </div>

                    <!-- 3 in a line: Bottom Action Buttons -->
                    ...
                    <button type="submit" class="btn-gradient" style="font-size: 16px; padding: 14px;">💾 Save Message & Action Settings</button>
                </form>
            </div>
        </div>
    </body>
...
# --- 4. Category: 17 VIP Packs (3 in a Line) ---
...
async def packs_page(request: Request):
    ...
    for idx in range(17):
        ...
        <div style="background: rgba(7, 9, 19, 0.75); border: 1px solid rgba(168, 85, 247, 0.25); border-radius: 10px; padding: 14px;">
            <h4 style="color: var(--cyan-glow); margin: 0 0 8px 0; font-family:'Orbitron',sans-serif; font-size:12px;">🟢 Button #{idx + 1}</h4>
            ...
            <label style="color: #94a3b8; font-size: 11px;">Bulk Video URLs (One per line):</label>
    ...
    <html>
    <head>
        <title>Packs — Nagato Panel</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        {THEME_CSS}
    </head>
    <body>
        ...
        <div class="main-wrapper">
            <div class="navbar">
                <div class="nav-brand">
                    <button class="nav-toggle-btn" onclick="toggleSidebar()">☰</button>
                    <span>📦 17 VIP Category Packs</span>
                </div>
                <div class="status-badge">● ONLINE</div>
            </div>
            ...
            <div class="dashboard-container">
                <form action="/admin/save-packs" method="post">
                    ...
                    <button type="submit" class="btn-gradient" style="font-size: 16px; padding: 16px; margin-top: 10px;">💾 Save All 17 Packs</button>
                </form>
            </div>
        </div>
    </body>
