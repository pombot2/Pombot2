...
async def save_content_endpoint(request: Request):
    ...
    for idx in range(17):
        ...
        button_details[str(idx)] = {
            "pack": form.get(f"import asyncio
import json
import logging
import os
import random
...
# --- Cyber Background Canvas Script & CSS Template ---
THEME_CSS = """
<style>
...
async def admin_dashboard(request: Request):
    ...
    for idx in range(17):
        ...
        button_sections_html += f"""
        <div style="background: rgba(7, 9, 19, 0.7); border: 1px solid rgba(168, 85, 247, 0.25); border-radius: 10px; padding: 14px; margin-bottom: 14px;">
            <h4 style="color: var(--cyan-glow); margin: 0 0 10px 0; font-family:'Orbitron',sans-serif; font-size:13px;">🟢 Green Button #{idx + 1}</h4>
            ...
            <label style="color: #94a3b8; font-size: 12px;">Bulk Video URLs (One per line):</label>
        ...
        <div class="main-wrapper">
            ...
            <div class="dashboard-container">
                ...
                <div class="cyber-card">
                    ...
                    <form action="/admin/save-token" method="post">
                        <input type="text" name="token" value="{cfg['token']}" placeholder="123456:ABC-DEF1234..." required />
                        <button type="submit" class="btn-gradient">Update & Restart Bot Engine</button>
                    </form>
                    <form action="/admin/stop-bot" method="post" style="margin-top: 8px;">
                ...
                <form action="/admin/save-content" method="post">
                    <!-- UPI Setup -->
                    <div class="cyber-card">
                        <h3>2. Auto UPI & Receipt Forwarding</h3>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                            <div>
                        ...
                        <label>Admin Telegram Chat ID (For Forwarding Screenshots):</label>
                        <input type="text" name="admin_chat_id" value="{cfg['admin_chat_id']}" placeholder="e.g. 123456789" />
                    </div>

                    <!-- Start Message -->
                    <div class="cyber-card">
                        <h3>3. Start Message Configuration</h3>
                        <label>1st Message: Image Album URLs (One URL per line):</label>
                        <textarea name="welcome_images" rows="3">{welcome_images_text}</textarea>

                        <label>2nd Message: Caption:</label>
                        <textarea name="welcome_caption" rows="3">{cfg['welcome_caption']}</textarea>

                        <label>🔵 Payment Page Back Button Text:</label>
                        <input type="text" name="back_button_text" value="{cfg['back_button_text']}" required />
                    </div>

                    <!-- Bottom 3 Design Buttons -->
                    <div class="cyber-card">
                        <h3>4. Bottom 3 Design Buttons</h3>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                            <div>
                                <label style="color: var(--cyan-glow);">Button 1 (Left - Blue):</label>
                                <input type="text" name="btn_how_to_use" value="{cfg['btn_how_to_use']}" required />
                                <label>Alert Popup Text:</label>
                                <input type="text" name="msg_how_to_use" value="{cfg['msg_how_to_use']}" required />
                            </div>
                            <div>
                                <label style="color: #f87171;">Button 2 (Right - Red):</label>
                                <input type="text" name="btn_report_issue" value="{cfg['btn_report_issue']}" required />
                                <label>Alert Popup Text:</label>
                                <input type="text" name="msg_report_issue" value="{cfg['msg_report_issue']}" required />
                            </div>
                        ...
                        <label style="color: var(--cyan-glow); margin-top: 10px;">Button 3 (Bottom - Full Width):</label>
                        <input type="text" name="btn_language" value="{cfg['btn_language']}" required />
                        <label>Alert Popup Text:</label>
                        <input type="text" name="msg_language" value="{cfg['msg_language']}" required />
                    </div>
                    ...
                    <!-- 17 Green Buttons -->
                    <div class="cyber-card">
                        <h3>5. 17 Green Buttons (Per-Button Videos & Prices)</h3>
                        {button_sections_html}
                    </div>
