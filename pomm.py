...
# --- Telegram Handlers with Safe Markdown Delivery ---
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.warning(f"Markdown parse error in welcome caption ({e}). Sending plain text...")
        await context.bot.send_message(
            chat_id=chat_id,
            text=caption_text,
            reply_markup=build_main_keyboard(cfg)
    ...
    # Step 1: Click Green Button -> Deliver Videos + Pack Details Card
        ...
        for vid in valid_videos:
            try:
                ...
                logger.error(f"Failed sending video ({vid}): {e}")
                try:
                    ...
                    failed_urls.append(vid)

        # Fallback to interactive clickable buttons for external streaming sites
        if failed_urls:
            watch_keyboard = []
            for i, link in enumerate(failed_urls, 1):
                watch_keyboard.append([InlineKeyboardButton(f"▶️ Watch Sample Video {i}", url=link)])
