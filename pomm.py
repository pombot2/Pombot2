...
# Stores active admin upload sessions: {chat_id: target_button_index_0_based}
...
# ==========================================================
# ADMIN BULK IN-BOT MEDIA UPLOAD COMMANDS
# ==========================================================
...
async def handle_admin_upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("⚠️ Usage: `/upload <number 1-17>`\nExample: `/upload 1` to assign media to Button 1.", parse_mode="Markdown")
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Step 1: Click Green Button -> Deliver Videos/Photos + Pack Details Card
        ...
        for media_item in valid_videos:
            ...
            # 2. Attempt sending as photo (for bulk admin image uploads)
            if not sent:
                try:
            ...
            # 3. Attempt sending as generic document / animation file
            if not sent:
                try:
            ...
            # 4. If it's an external web URL that failed, record for button link fallback
...
async def handle_incoming_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Check if Admin is in active Bulk Upload Mode
    ...
    # 2. Check if user is uploading payment screenshot
    session = pending_verifications.get(chat_id)
    ...
    # 3. Fallback: If admin sends media outside upload mode, reply with file_id
    if is_admin(chat_id):
        f_id = None
