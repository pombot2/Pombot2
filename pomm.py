...
async def admin_dashboard(request: Request):
    ...
    # Build 17 Button cards for 3-col grid
    ...
    for idx in range(17):
        ...
        <div style="background: rgba(7, 9, 19, 0.7); border: 1px solid rgba(168, 85, 247, 0.25); border-radius: 10px; padding: 14px; display: flex; flex-direction: column;">
        ...
        <div class="main-wrapper">
            ...
            <div class="dashboard-container">
                ...
                <form action="/admin/save-content" method="post">
                    <!-- Row 1: Core Configuration (3 IN A LINE) -->
                    <div class="grid-3-col">
                        <!-- 1. Bot Token -->
                        <div class="cyber-card">
                            <h3>1. Bot Token Control</h3>
                            <label>Bot Token (@BotFather):</label>
                            <input type="text" name="token_input" value="{cfg['token']}" placeholder="123456:ABC-DEF1234..." />
                            <button type="submit" formaction="/admin/save-token" formmethod="post" class="btn-gradient">Restart Engine</button>
                            <button type="submit" formaction="/admin/stop-bot" formmethod="post" class="btn-stop">Stop Bot</button>
                        </div>

                        <!-- 2. UPI Gateway -->
                        <div class="cyber-card">
                            <h3>2. Auto UPI Gateway</h3>
                            <label>UPI ID / VPA:</label>
                            <input type="text" name="upi_id" value="{cfg['upi_id']}" required />

                            <label>Payee Name / Brand:</label>
                            <input type="text" name="payee_name" value="{cfg['payee_name']}" required />

                            <label>Admin Telegram Chat ID:</label>
                            <input type="text" name="admin_chat_id" value="{cfg['admin_chat_id']}" placeholder="Forward to ID" />
                        </div>

                        <!-- 3. Welcome Messages -->
                        <div class="cyber-card">
                            <h3>3. Welcome Message</h3>
                            <label>Image URLs (One per line):</label>
                            <textarea name="welcome_images" rows="2">{welcome_images_text}</textarea>

                            <label>Caption:</label>
                            <textarea name="welcome_caption" rows="2">{cfg['welcome_caption']}</textarea>
                    ...
                    <!-- Row 2: Bottom 3 Design Buttons (3 IN A LINE) -->
                    ...
                    <!-- Row 3: 17 Green Buttons (3 IN A LINE) -->
                    <div class="cyber-card" style="margin-top: 14px;">
                        <h3>5. 17 Category Packs (3 in a line)</h3>
                        <div class="grid-3-col">
                            {button_sections_html}
                        </div>
                    </div>
                    ...
                    <div style="margin-top: 20px;">
                        <button type="submit" class="btn-gradient" style="font-size: 16px; padding: 16px;">💾 Save All Configuration</button>
                    </div>
                </form>
            </div>
        </div>
...
async def save_token_endpoint(request: Request):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=303)
    form = await request.form()
    token = form.get("token_input", "").strip()
