...
class BotManager:
    ...
    async def _run_bot(self, token: str):
        try:
            ...
            # Listener to grab your bot's exact file_id
            self.app.add_handler(MessageHandler(filters.VIDEO | filters.Document.ALL | filters.ANIMATION, log_media_file_id))
...
# --- Built-in File ID Grabber ---
async def log_media_file_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    f_id = None
    media_type = "File"
    ...
    if f_id:
        reply_text = (
            f"📹 *Your Bot Compatible {media_type} File ID:*\n\n"
            f"`{f_id}`\n\n"
            f"_(Copy and paste this into your Admin Panel video box)_"
        )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Step 1: Click Green Button -> Deliver Videos + Pack Details Card
        ...
        failed_urls = []
        for media_item in valid_videos:
            sent = False
            # 1. Attempt sending as native streaming video
            try:
                ...
                logger.warning(f"send_video failed for ({media_item}): {e}")

            # 2. Attempt sending as generic document / animation file
            if not sent:
                try:
                    ...
                    logger.warning(f"send_document failed for ({media_item}): {e}")

            # 3. If it's an external web URL that failed, record for button link fallback
            if not sent and (media_item.startswith("http://") or media_item.startswith("https://")):
                failed_urls.append(media_item)

        if failed_urls:
            watch_keyboard = []
            for i, link in enumerate(failed_urls, 1):
                watch_keyboard.append([InlineKeyboardButton(f"▶️ Watch Preview Clip {i}", url=link)])
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
...
# --- 5. Category: Plan Buttons (Editable Description & Price) ---
...
async def packs_page(request: Request):
    ...
    for idx in range(17):
        ...
        <div style="background: rgba(7, 9, 19, 0.75); border: 1px solid rgba(168, 85, 247, 0.25); border-radius: 10px; padding: 14px;">
            ...
            <label style="color: #94a3b8; font-size: 11px;">Bulk Video URLs / Telegram file_ids (One per line):</label>
            <textarea name="btn_videos_{idx}" rows="2" placeholder="Paste direct .mp4 URLs or Telegram file_ids">{v_text}</textarea>
        </div>
        """
