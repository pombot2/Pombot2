    ...
    if c.fetchone()[0] == 0:
        ...
        default_details = {
            str(i): {
                "pack": f"VIP Pack {i + 1}",
                "price": "299",
                "desc": "High speed streaming access."
            } for i in range(17)
        }
        ...
        c.execute("""
            INSERT INTO settings (
                ...
                button_videos_json, button_details_json, home_button_text, back_button_text, upi_id, payee_name
            ) VALUES (1, '', ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
...
def get_settings():
    ...
    c.execute("""
        SELECT token, welcome_images_json, welcome_caption, buttons_json, 
               ...
               back_button_text, upi_id, payee_name 
...
# --- Automatic UPI QR Code URL Generator ---
def generate_upi_qr_url(upi_id: str, payee_name: str, amount: str, note: str) -> str:
    """Creates a direct dynamic UPI QR link encoded with payee, price, and note."""
    clean_amount = "".join(c for c in amount if c.isdigit() or c == '.') or "0"
    params = {
        "pa": upi_id.strip(),
...
def build_main_keyboard(cfg):
    ...
    # 1 Blue Home Button (Second Message)
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Message 2: Text Caption + 17 Green Buttons + 1 Blue Home Button
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "btn_home":
        ...
        # 1. Send bulk videos assigned to this button
        ...
        btn_idx = query.data.replace("btn_show_qr_", "")
        details = cfg["button_details"].get(btn_idx, {
            "pack": "VIP Pack",
            "price": "0",
            "desc": ""
        ...
        if cfg["upi_id"]:
            # Auto-generate dynamic QR code with exact amount and note
            qr_url = generate_upi_qr_url(
                upi_id=cfg["upi_id"],
                payee_name=cfg["payee_name"],
                amount=details.get("price", "0"),
                note=f"Payment for {details.get('pack', 'VIP Pack')}"
            )

            caption = (
                f"📲 *Scan to Pay ₹{details.get('price', '0')}*\n\n"
                f"👤 *UPI ID:* `{cfg['upi_id']}`\n"
                f"📦 *Item:* {details.get('pack', '')}\n\n"
                f"_Scan with Google Pay, PhonePe, Paytm, or BHIM. Send screenshot receipt here when done._"
            )
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text="⚠️ *Admin has not configured a UPI ID yet.*",
                reply_markup=qr_markup,
                parse_mode="Markdown"
            )
...
async def admin_dashboard():
    ...
    # Build 17 Button configuration cards
    ...
    for idx in range(17):
        ...
        details = cfg["button_details"].get(str(idx), {
            ...
            "price": "299",
            "desc": "High quality video collection."
        })
    ...
    <body>
        <div class="container">
            <h1>⚙️ Bot Console (Auto UPI QR Engine)</h1>
            ...
            <form action="/admin/save-content" method="post">
                ...
                <div class="card" style="border: 1px solid #38bdf8;">
                    <h3 style="color: #38bdf8; margin-top: 0;">⚡ Auto-Generated Payment QR Settings</h3>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <div>
                            <label>Your UPI ID / VPA (e.g. username@okhdfcbank):</label>
                    ...
                    <p style="font-size: 12px; color: #94a3b8; margin: 0;">The QR code will automatically encode your UPI ID and the exact price set for each button.</p>
                </div>
                ...
                <div class="card">
                    <h3>3. 17 Items Configuration (Pack Name, Price, Description, Videos)</h3>
                    {button_sections_html}
                </div>
