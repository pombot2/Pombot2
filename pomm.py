    ...
    if c.fetchone()[0] == 0:
        ...
        default_details = {
            str(i): {
                ...
                "link": "https://t.me/YourPrivateChannel"
            } for i in range(17)
        }
...
async def admin_dashboard_view(request: Request, message: str | None = None):
    ...
    if not users_rows:
        users_rows = '<tr><td colspan="3" class="py-12 text-center text-purple-400 text-xs">No registered users in database yet.</td></tr>'
    ...
    for idx in range(17):
        ...
        pack_cards_html += f"""<div class="glass-card rounded-2xl p-4 flex flex-col justify-between space-y-3 border border-purple-900/40">
            ...
            <div class="space-y-2 text-xs">
                ...
                <div>
                    <label class="block text-[10px] font-mono text-purple-300 uppercase">Bulk Media (file_id or URL)</label>
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden relative">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto relative z-10">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <div id="tab-dashboard" class="tab-content space-y-6">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    ...
                    <div class="glass-card rounded-2xl p-4 flex flex-col justify-between hover:border-fuchsia-500/40 transition">
                        ...
                        <div>
                            <span class="font-tech text-2xl md:text-3xl font-extrabold text-white tracking-tight">&#8377;{revenue:.2f}</span>
                    ...
                    <div class="glass-card rounded-2xl p-4 flex flex-col justify-between neon-border-pink relative overflow-hidden">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-purple-400 mt-1 font-mono">
                                Valid till: <span class="text-slate-300">{cfg['license_expiry']}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form method="POST" action="/admin/broadcast/send" class="glass-card rounded-2xl p-6 space-y-5">
                    <div class="flex items-center gap-3 border-b border-purple-900/40 pb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-purple-400/80">Send announcement message or media to all users.</p>
                        </div>
                    </div>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-semibold text-purple-300 uppercase tracking-wider mb-2 font-mono">Broadcast Message *</label>
                            <textarea name="broadcast_message" rows="4" required placeholder="Type announcement..."
                        ...
                        <div>
                            ...
                            <input type="text" name="broadcast_photo" placeholder="https://... or AgAC..."
                        ...
                        <div>
                            ...
                            <input type="text" name="broadcast_video" placeholder="https://... or BAAC..."
                                   class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-400 transition">
                        </div>
                    </div>
                ...
                <form method="POST" action="/admin/save-bot-settings" class="glass-card rounded-2xl p-6 space-y-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="upi_id" value="{cfg['upi_id']}" required placeholder="thesalesgod@nyes"
                        ...
                        <div>
                            ...
                            <input type="text" name="payee_name" value="{cfg['payee_name']}" required placeholder="Trusted Seller"
                        ...
                        <div>
                            ...
                            <input type="text" name="admin_chat_id" value="{cfg['admin_chat_id']}" placeholder="e.g. 6528792525"
                ...
                <form method="POST" action="/admin/save-message-settings" class="glass-card rounded-2xl p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="welcome_caption" rows="4" required
                                  class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-400 transition">{cfg['welcome_caption']}</textarea>
                    </div>
                    ...
                    <div class="border-t border-purple-900/40 pt-4 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="p-3 bg-[#070410]/60 border border-purple-900/40 rounded-xl space-y-2">
                                ...
                                <input type="text" name="msg_how_to_use" value="{cfg['msg_how_to_use']}" required placeholder="Popup Alert Message" class="w-full bg-black/50 border border-purple-900/60 rounded-lg px-3 py-1.5 text-xs text-slate-300">
                            </div>
                            <div class="p-3 bg-[#070410]/60 border border-purple-900/40 rounded-xl space-y-2">
                                ...
                                <input type="text" name="msg_report_issue" value="{cfg['msg_report_issue']}" required placeholder="Popup Alert Message" class="w-full bg-black/50 border border-purple-900/60 rounded-lg px-3 py-1.5 text-xs text-slate-300">
                            </div>
                        </div>
                        ...
                        <div class="p-3 bg-[#070410]/60 border border-purple-900/40 rounded-xl space-y-2">
                            ...
                            <input type="text" name="msg_language" value="{cfg['msg_language']}" required placeholder="Popup Alert Message" class="w-full bg-black/50 border border-purple-900/60 rounded-lg px-3 py-1.5 text-xs text-slate-300">
                        </div>
                    </div>
            ...
            <!-- TAB: 17 PLAN BUTTONS (WITH CHOOSE FILE & ACCESS LINK) -->
            <div id="tab-plans" class="tab-content space-y-5 hidden">
                <div class="glass-card rounded-2xl p-5 space-y-2 border-l-4 border-cyan-400">
                    <h3 class="font-tech text-xs text-white font-bold tracking-wider">💡 Bulk Media &amp; Access Delivery Links</h3>
                    <p class="text-xs text-purple-300/80 font-mono">
                        You can choose files directly below, type URLs/file_ids, or send <code class="text-cyan-400">/upload 1</code> inside Telegram. 
...
async def change_order_status(order_id: int, new_status: str, request: Request):
    ...
    if new_status in ["Paid", "Rejected", "Pending"]:
        ...
        # Deliver optional link to user when status is set to Paid
...
async def save_packs_settings(request: Request):
    ...
    for idx in range(17):
        ...
        # Process direct file uploads (Choose File)
